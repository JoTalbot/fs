...
class ReplicaPolicy:
    ...
    def plan(
        ...
        *,
        ...
        healthy_present = [
            c for c in candidate_list if c.node_id in present and c.healthy
        ]
        failed_present = {
            c.node_id
            for c in candidate_list
            if c.node_id in present and not c.healthy
        }
        existing_domains = {c.failure_domain for c in healthy_present}
        eligible = [
            c
            for c in candidate_list
            if c.healthy and c.capacity_available >= 0 and c.node_id not in present
        ]
        ...
        for candidate in sorted(
            eligible,
            key=lambda c: (
                -int(c.failure_domain not in used_domains),
