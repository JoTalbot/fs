...
from .production_adapters import AuthenticatedTransport, DurableAdmissionCoordinator, KeyAdmission, NodeAdmission, SecureKeyStore
...
def run_adapter_conformance(*, key_store_factory: Callable[[], SecureKeyStore], transport_factory: Callable[[], AuthenticatedTransport], key_admission_factory: Callable[[], KeyAdmission], node_admission_factory: Callable[[], NodeAdmission], coordinator_factory: Callable[[], DurableAdmissionCoordinator]) -> tuple[str, ...]:
    ...
    try: store.load("k1")
    except (KeyError, LookupError, ValueError, TypeError, PermissionError): pass
    else: raise AdapterConformanceError("unknown key must not be loadable")
    try: store.store("", b"qualification-key")
    except (ValueError, TypeError): pass
    else: raise AdapterConformanceError("empty key id must be rejected")
    ...
    try: store.store("k-empty", b"")
    except (ValueError, TypeError): pass
    else: raise AdapterConformanceError("empty key material must be rejected")
    ...
    try: transport.send("node-a", b"qualification")
    except (PermissionError, RuntimeError): pass
    else: raise AdapterConformanceError("unauthenticated transport send must be rejected")
    try: transport.authenticate("")
    except (ValueError, TypeError, PermissionError, RuntimeError): pass
    else: raise AdapterConformanceError("empty peer identity must be rejected")
    ...
    try: transport.send("node-b", b"qualification")
    except (PermissionError, RuntimeError): pass
    else: raise AdapterConformanceError("transport must reject sends to an unauthenticated peer")
    ...
    else: raise AdapterConformanceError("empty coordinator resource must be rejected")
